"""
SubringTransformer is a struct which support the number theoretic transform (NTT) over the _subring.
You can generate a SubringTransformer over a ring Z[X]/(Φₘ(X), Q) with length N = (m-1)/d by ```SubringTransformer(m, d, Q)```
"""
struct SubringTransformer
    Q::Modulus
    m::Int64
    d::UInt64
    N::Int64
    g::Int64
    minv::UInt64
    Ψ::Vector{UInt64}
    Ψinv::Vector{UInt64}
    buff::Vector{UInt64}
    gpowN::Vector{Int32}
    ntter::CyclicTransformer2a3b5c7d

    function SubringTransformer(m::Int64, d::Int64, Q::Modulus)
        @assert isprime(m) "$m is not a prime number."
        @assert (m - 1) % d == 0 "$d should divide $(m-1)."
        @assert Q.Q % m == 1 "$(Q.Q) does not have enough factors to perform NTT."

        g = primitive_root_finder(m)
        N = (m - 1) ÷ d

        convlen = is2a3b5c7d(N) ? N : next2a3b5c7d(2N - 1)
        ntter = CyclicTransformer2a3b5c7d(convlen, Q)

        ξ = primitive_root_finder(Q)
        ζ = ith_root_from_primitive_root(m, ξ, Q)
        ζinv = powermod(ζ, m - 1, Q)

        Ψ, Ψinv = zeros(UInt64, convlen), zeros(UInt64, convlen)
        gᴺ = powermod(g, N, m)
        ζgʲᴺ, ζinvgʲᴺ = ζ, ζinv
        @inbounds for _ = 1:d
            ζgʲᴺ, ζinvgʲᴺ = powermod(ζgʲᴺ, gᴺ, Q), powermod(ζinvgʲᴺ, gᴺ, Q)
            ζgʲᴺgⁱ, ζinvgʲᴺgⁱ = ζgʲᴺ, ζinvgʲᴺ
            for i = 1:N
                Ψ[i] = _add(Ψ[i], ζgʲᴺgⁱ, Q)
                Ψinv[i] = _add(Ψinv[i], ζinvgʲᴺgⁱ, Q)
                ζgʲᴺgⁱ, ζinvgʲᴺgⁱ = powermod(ζgʲᴺgⁱ, g, Q), powermod(ζinvgʲᴺgⁱ, g, Q)
            end
        end

        _ntt!(Ψ, ntter)
        _ntt!(Ψinv, ntter)

        buff = Vector{UInt64}(undef, convlen)
        gpowN = Int32[powermod(g, i, m) for i = 0:N-1]

        new(Q, m, d, N, g, invmod(m, Q), Ψ, Ψinv, buff, gpowN, ntter)
    end
end

@views function _ntt_to!(res::AbstractVector{UInt64}, a::AbstractVector{UInt64}, ntter::SubringTransformer)
    buff, N, Q = ntter.buff, ntter.N, ntter.Q

    buff[1] = a[1]
    @. buff[2:N] = a[N:-1:2]
    @. buff[N+1:end] = 0

    _ntt!(buff, ntter.ntter)
    _lazy_Bmul_to!(buff, buff, ntter.Ψ, Q)
    _intt!(buff, ntter.ntter)

    length(buff) ≠ N && _add_to!(buff[1:N-1], buff[1:N-1], buff[N+1:2N-1], Q)
    @. res = buff[1:N]
end

_ntt!(a::AbstractVector{UInt64}, ntter::SubringTransformer) = _ntt_to!(a, a, ntter)

@views function _intt_to!(res::AbstractVector{UInt64}, a::AbstractVector{UInt64}, ntter::SubringTransformer)
    buff, N, d, minv, Q = ntter.buff, ntter.N, ntter.d, ntter.minv, ntter.Q

    buff[1] = a[1]
    @. buff[2:N] = a[N:-1:2]
    @. buff[N+1:end] = 0

    _ntt!(buff, ntter.ntter)
    _lazy_Bmul_to!(buff, buff, ntter.Ψinv, ntter.Q)
    _intt!(buff, ntter.ntter)

    length(buff) ≠ N && _add_to!(buff[1:N-1], buff[1:N-1], buff[N+1:2N-1], Q)

    t = UInt64(0)
    @inbounds for i = 1:N
        t = _add(t, a[i], Q)
    end
    _sub_to!(buff[1:N], buff[1:N], _Bmul(t, d, Q), Q)
    _Bmul_to!(buff[1:N], minv, buff[1:N], Q)
    @. res = buff[1:N]
end

_intt!(a::AbstractVector{UInt64}, ntter::SubringTransformer) = _intt_to!(a, a, ntter)