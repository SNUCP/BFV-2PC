struct IntPackerPow2
    ...
    buff::Vector{UInt64}
    ntter::CyclotomicTransformerPow2
    ...
    function IntPackerPow2(m::Integer, p::Integer, r::Integer)
        ...
        if p % 8 == 1
            ...
            ntter = CyclotomicTransformerPow2(k << 1, pr)
...
find_and_save_factors(m::Int64, p::Int64, r::Int64) = begin
    ...
    save_factors("m$(m)p$(p)r$(r)", cube, cubegen, factors, resol)
    cube, cubegen, factors, resol
end

save_factors(paramname::String, cube::Vector{Int64}, cubegen::Vector{Int64}, factors::Vector{Vector{Int64}}, resol::Vector{Int64}) = begin
    open(String(@__DIR__) * "/factors.jl", "a") do file
        println(file, paramname, " = (", cube, ", ", cubegen, ", ", factors, ", ", resol, ")")
    end
end
    ...
    factors = open(String(@__DIR__) * "/factors.jl") do file
        for str in eachline(file)
            if occursin("m$(m)p$(p)r$(r)", str)
                ...
                resol = parse.(Int64, split(str[idx4+4:idx5-2], ","))
...
struct IntPackerSubring
    ...
    cubegen::Vector{Int64}
    P::Vector{Modulus}
    ntterP::Vector{CyclicTransformer2a3b5c7d}
    ...
    @views function IntPackerSubring(m::Integer, d::Integer, p::Integer, r::Integer)
        ...
        ntterP = CyclicTransformer[CyclicTransformer(convlen, Pi) for Pi = P]
...
@views function pack_to!(res::AbstractVector{UInt64}, a::AbstractVector{UInt64}, packer::IntPackerSubring)
    ...
    @assert length(a) == k "The length of the input message should be equal to the number of slots."
    @assert length(res) == N "The length of the output message should be equal to the polynomial degree."
...
"""
IntPacker is an abstract type which supports packing.
"""
