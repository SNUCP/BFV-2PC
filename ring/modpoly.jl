...
_add_to!(res::AbstractVector{UInt64}, x::AbstractVector{UInt64}, y::UInt64, isntt::Bool, eval::_PolyEvaluatorNTT) = begin
    ...
    elseif typeof(eval.ntter) == SubringTransformer
...
_automorphism!(idx::Int64, x::AbstractVector{UInt64}, isntt::Bool, eval::_PolyEvaluatorNTT) = begin
    if typeof(eval.ntter) == SubringTransformer
        _automorphism!(idx, x, isntt, eval.ntter)
    elseif typeof(eval.ntter) == CyclotomicTransformerPow2
        _automorphism!(idx, x, isntt, eval.autbuff, eval.Q)
    elseif typeof(eval.ntter) == CyclotomicTransformerArb
...
struct _PolyEvaluatorWord
    ...
    @views function _PolyEvaluatorWord(param::RingParam, Q::Modulus)
        ...
        ntterP = [Transformer(param, Pi) for Pi = P]
        m, N = param.m, param.N
...
_add_to!(res::AbstractVector{UInt64}, x::AbstractVector{UInt64}, y::UInt64, isntt::Bool, eval::_PolyEvaluatorWord) = begin
    if typeof(eval.ntterP[1]) == SubringTransformer
        _sub_to!(res, x, y, eval.Q)
    else
        @. res = x
        res[1] = _add(res[1], y, eval.Q)
...
_automorphism!(idx::Int64, x::AbstractVector{UInt64}, isntt::Bool, eval::_PolyEvaluatorWord) = begin
    ...
    elseif typeof(eval.ntterP[1]) == CyclotomicTransformerPow2
...
Base.:getindex(eval::PolyEvaluator, idx::UnitRange) = PolyEvaluator(eval.param, eval.evals[idx], eval.moduli[idx])
...
Base.copy(dst::ModScalar) = ModScalar(copy(dst.vals))
...
struct ModPoly
    ...
    ModPoly(coeff::Vector{<:Union{Int64,UInt64}}, eval::PolyEvaluator) = begin
        ...
        new(coeffs, length(coeff), Ref(false))
    end

    ModPoly(coeff::Vector{BigInt}, eval::PolyEvaluator) = begin
        coeffs = Vector{Vector{UInt64}}(undef, length(eval))
...
Base.:getindex(x::ModPoly, idx::UnitRange) = ModPoly(x.coeffs[idx], x.isntt[])
...
function to_big(x::ModPoly, eval::PolyEvaluator)
    ...
    @assert !x.isntt[] "Polynomial should be in coefficient form."
    ...
    @inbounds for i = eachindex(eval)
        ...
        Qtildeinv = invmod(UInt64(Qtilde % Qi.Q), Qi.Q)
        for j = 1:N
            res[j] += _mul(x.coeffs[i][j], Qtildeinv, eval[i]) * Qtilde
        end
    end
